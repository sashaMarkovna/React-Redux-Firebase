...
export const signUp = (newUser) => {
  return (dispatch, getState, { getFirebase, getFirestore }) => {
    ...
    if (newUser.gender === 'male') {
      avatar = userDefaultSettings.maleUserAvatar;
    } else if (newUser.gender === 'female') {
      avatar = userDefaultSettings.femaleUserAvatar;
    } else {
      avatar = userDefaultSettings.unknownUserAvatar;
    }
...
export const changeUserEmailOrPassword = (newCredentials) => {
  return (dispatch, getState, { getFirebase }) => {
    ...
    const cred = firebase
            .auth
            .EmailAuthProvider
            .credential(currentUser.email, newCredentials.password);
    if (newCredentials.update === 'email') {
      currentUser.reauthenticateAndRetrieveDataWithCredential(cred)
                .then(() => {
                  currentUser.updateEmail(newCredentials.newEmail)
                    .then(() => {
                      dispatch({ type: 'EMAIL_UPDATE_SUCCESS' });
                    }).catch((error) => {
                      dispatch({ type: 'EMAIL_UPDATE_EMAIL_ERROR', error });
                    });
                }).catch((error) => {
                  dispatch({ type: 'EMAIL_UPDATE_PASSWORD_ERROR', error });
                });
    } else {
      currentUser.reauthenticateAndRetrieveDataWithCredential(cred)
                .then(() => {
                  currentUser.updatePassword(newCredentials.newPassword)
                    .then(() => {
